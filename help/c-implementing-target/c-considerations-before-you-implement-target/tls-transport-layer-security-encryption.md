...
>[!IMPORTANT]
>
>After February 2020, Adobe Target will no longer support TLS 1.1 encryption for the Visual Experience Composer (VEC), Enhanced Experience Composer (EEC), activity delivery, APIs, etc. Please upgrade to TLS 1.2 before February 2002 to avoid any issues.
...
Until now, Adobe Target's [Enhanced Experience Composer](../../c-experiences/experiences.md#section_34265986611B4AB8A0E4D6ACC25EF91D) (EEC) used TLS 1.0 by default. After February 2020, Target is moving to TLS 1.2 by default.

Adobe will be moving customers in a phased manner to TLS 1.2. For those, whose domains are already 1.2 compliant, we will move them to TLS 1.2 without any changes needed from you. Most customer domains already support TLS 1.2; however, if your domain does not support TLS 1.2, we will keep those domains on TLS 1.0 like today (until February 2020).
...
As you plan for this change, consider the following (note that the February 2020 deadline affects all of these items):
...
Starting February 2020, Target APIs will no longer support TLS 1.0 encryption. Customers who access the API should verify that they will not be impacted.
...
1. Open the Security tab, then examine the TLS version information under Connection:
   ...
   ![TLS version details](/help/c-implementing-target/c-considerations-before-you-implement-target/assets/chrome-tls-version.png)
